    ...
    public partial class NewForm : Form
    {
        ...
        void removeTheSelectedItemFromListView()
        {
            ...
            if (actual_order.SelectedItems.Count > 0)
            {
                ...
                // Find the selected item in the menuPositions list and remove it
                string selectedItemName = selectedItem.Text;

                try
                {
                    MenuPosition mp = menuPositions.Where(item => item.Nazwa == selectedItemName).FirstOrDefault();
                    menuPositions.Remove(mp);
                }
                catch (Exception ex) 
                {
